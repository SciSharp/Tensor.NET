...
function cmake_build() {
    ...
    mkdir -p $build_dir
    cd "build"
    bash -c "cmake .. -DNN_WITH_TEST=$NN_WITH_TEST -DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DNN_CROSS_COMPILE=ON ${EXTRA_CMAKE_ARGS}"
    bash -c "make -j${MAX_PARALLEL_JOBS}"
    cd ".."
    bash -c "cp ${build_dir}/lib/${package_name}.dll ${output_dir}/${package_name}.dll"
