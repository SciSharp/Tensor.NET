...
void assert_same_view(const Tensor &a, const Tensor &b, float err = 0.000001f) {
  ...
  auto lptr = a.ptr<ctype>();
  auto rptr = b.ptr<ctype>();
  for (nn_size i = 0; i < (a.layout.ndim >= 4 ? a.layout[3] : 1); i++) {
    for (nn_size j = 0; j < (a.layout.ndim >= 3 ? a.layout[2] : 1); j++) {
      for (nn_size k = 0; k < (a.layout.ndim >= 2 ? a.layout[1] : 1); k++) {
        for (nn_size p = 0; p < a.layout[0]; p++) {
