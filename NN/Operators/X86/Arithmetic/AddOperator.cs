...
namespace NN.Native.Operators.X86.Arithmetic
{
    ...
    public class AddOperator<T> : IAddOperator<T> where T : unmanaged, IAdditionOperators<T, T, T>, INumberBase<T>
    {
...
#else
    public class AddOperator<T, THandler>: IAddOperator<T> where T : unmanaged where THandler: INativeDTypeHandler<T>, new()
    {
        ...
        {
            var res = new NativeArray<T>(IAddOperator<T>.DeduceLayout(a._layout, b._layout), memoryManager);
        ...
        {
            var res = new NativeArray<T>(IAddOperator<T>.DeduceLayout(a._layout), memoryManager);
